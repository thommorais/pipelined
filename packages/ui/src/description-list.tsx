...
export function DescriptionTerm({ className, ...props }: React.ComponentPropsWithoutRef<'dt'>) {
	return (
		<dt
			...
			className={clsx(
				className,
				'col-start-1 border-black/5 border-t pt-3 text-zinc-500 first:border-none sm:border-black/5 sm:border-t sm:py-3 dark:border-white/5 dark:text-zinc-400 sm:dark:border-white/5',
			)}
		/>
	)
}
...
export function DescriptionDetails({ className, ...props }: React.ComponentPropsWithoutRef<'dd'>) {
	return (
		<dd
			...
			className={clsx(
				className,
				'pt-1 pb-3 text-black sm:border-black/5 sm:border-t sm:nth-2:border-none sm:py-3 dark:text-white dark:sm:border-white/5',
			)}
		/>
	)
}
